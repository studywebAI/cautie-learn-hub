...
5. `[ ]` Een afgeronde toets-`assignment` (via `assignment_attempts`, al aanwezig met `score`/`max_score`) krijgt automatisch een `grade_sets`-rij in status **`grading`** (nieuwe status naast `draft/in_progress/completed`) zodra de eerste attempt binnen is — dat vult "to grade" in Grades. Akkoord, of moet dit pas gebeuren als de docent er zelf voor kiest (bv. een "start nakijken"-knop)?
6. `[ ]` Meerkeuze/gesloten vragen worden meteen gescoord (logica bestaat al, zie `computeStudentAnswerStats`). Open vragen krijgen per antwoord een status `pending` / `correct` / `incorrect` / `ai_suggested`. Docent kan per vraag of in bulk beoordelen; bij AI-suggestie kan de docent met één klik overnemen of overrulen.
7. `[ ]` Jij noemde eerder (bij het dashboard-gesprek) ook een **"foutgekeurd"-meldknop**: leerling kan aangeven "vraag 4c is fout nagekeken", docent krijgt dat als issue bij Grades. Hoort dat hier ook bij fase 1 (voor het cijfer definitief is) of pas na cijfer-publicatie als correctieverzoek?
...
11. `[ ]` Voorstel voor de bouw: dit hergebruikt de **al bestaande** `class_grading_presets`-tabel — geen nieuwe tabel nodig. `kind` breidt uit met een nieuwe waarde `'scale_template'`, en `config` bevat de bins, bv.:
    ...
    ```
    Cautie levert een paar kant-en-klare `system`-presets (NL, VS A-F, DE 1-6) die de docent kan kiezen en daarna zelf mag aanpassen (bins verschuiven), plus een lege "eigen systeem" optie waarbij de docent zelf bins/labels intypt. Akkoord met dit model?
