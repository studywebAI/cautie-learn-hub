...
// GET — open 'grading_dispute' events for this grade set's test, with
// question/student context, for the docent to review under Grades.
...
// POST — resolve a dispute: either 'reopen' (clear the answer's score so it
// reappears in the nakijken flashcard queue for re-review) or 'dismiss'.
export async function POST(
  ...
  try {
    ...
    const { data: event } = await supabase
      ...
      .eq('id', eventId)
      .eq('assignment_id', (await supabase.from('grade_sets').select('assignment_id').eq('id', gradeSetId).eq('class_id', classId).maybeSingle()).data?.assignment_id || '')
      .maybeSingle();
