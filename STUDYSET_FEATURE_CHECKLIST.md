...
Current: 3 steps (Basics incl. icon/color → Calendar → Sources). Target shape per the
user's spec — keep it ONE flow, on-theme, but restructure/extend steps:

- [x] Step: name + icon/color
- [x] Step: calendar — pick the days you can study (never times — pace is personal, AI/us can't predict it)
- [x] Step: Upload — **3-option scheme** shipped this round, real data throughout:
  - [x] Option 1 — **From your agenda** (`AgendaTemplatePicker`, queries the real
        `/api/agenda/feed`): shows the user's actual upcoming tests/assignments
        (title, class, subject, due date) — picking one seeds focus notes AND carries
        the real subject + exam date onto the studyset itself (`subject`/`exam_date`
        columns, not just inside `source_bundle`)
  - [x] Option 2 — **Pick a subject & chapter** (`SubjectTopicPicker`, walks the real
        `/api/subjects` → `.../chapters` → `.../paragraphs` hierarchy): cascading
        picker with optional multi-select paragraphs (max 6); seeds focus notes like
        "Focus on Biology → Chapter 4: Cell division. Specifically: §2 Mitosis, §3 Meiosis."
  - [x] Option 3 — **Upload your own**: the existing notes/paste/files/OneDrive flow,
        fully preserved and now optional-but-available alongside the other two
  - [x] A persistent "Linked to: …" chip shows the active agenda/curriculum link
        (with a Remove action) regardless of which method tab is focused — so users
        can combine "this is for Friday's test" with "and here are my own notes"
  - *Note: did not build deep document-content extraction from assignments/blocks for
    option 2 — that data is question/exercise content, not lecture material, so seeding
    real subject/chapter/paragraph context (which the AI can genuinely act on) was the
    honest "real, not mockup" choice rather than extracting the wrong kind of text.*
- [ ] Step: **Tool mindmap** — visual picker showing all output tools (quiz, flashcards,
      notes, …); clicking one reveals that tool's specific settings + question types
      — *not built: the wizard currently always produces a mixed quiz/flashcards/notes
      plan via the adaptive engine; a tool-picker step would need new per-tool generation
      paths and is a larger structural change saved for a follow-up*
- [x] Step: **Generate** — already real: `createStudyset()` calls
      `POST /api/studysets/[id]/generate` right after creation, which produces the
      actual day-by-day plan (mixed quiz/flashcards/notes tasks via the adaptive engine)
...
5. ~~Create-flow restructure: 3-option upload scheme (PAGE 2)~~ ✅ shipped (core)
   — added `AgendaTemplatePicker` (real `/api/agenda/feed` data: pick an upcoming test/
   assignment and the studyset inherits its real subject + exam date, plus generated
   focus notes) and `SubjectTopicPicker` (real `/api/subjects` → chapters → paragraphs
   cascade, generates "Focus on X → Chapter N: ... Specifically: §..." notes). Wired
   both into `app/(main)/tools/studyset/page.tsx` step 2 as a 3-card chooser
   (agenda / subject&chapter / upload your own) sitting on top of the existing
   notes/files flow — fully additive, the working linear wizard untouched, defaults
   to "upload" so nothing breaks. `createStudyset()` now also forwards the linked
   `subject`/`exam_date` to `POST /api/studysets` (already supported server-side) and
   stores a `linked_source` descriptor in `source_bundle.sources`.
   — Deliberately deferred (documented in PAGE 2 above): the **tool-mindmap step**
   (a visual per-tool picker with tool-specific settings) is a genuinely separate
   structural change — generation is currently always a mixed adaptive plan, not
   single-tool — so it's left as its own future step rather than rushed in here.
   The **Generate step** itself was already real (`POST /api/studysets/[id]/generate`
   fires right after creation), just not previously called out as "done" in the plan.
