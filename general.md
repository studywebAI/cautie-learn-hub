...
Ik splits het in 2 grote modules:

🕒 HISTORY (audit & veranderingsoverzicht)

Dit is vooral voor docenten + admins
Doel: transparantie, veiligheid, en herstelopties.

📜 1. History Dashboard (overzicht)

Wanneer je op History klikt zie je een logboek:

Date        User        Action
---------------------------------------
Feb 20 14:32  Mr. Smith  Edited grade (John Kennedy) 7 → 8
Feb 20 14:30  Mr. Smith  Published "Math Test 1"
Feb 19 09:10  Ms. Lee    Created "Quiz Week 2"
...
🧩 2. Per Grade Set History

Als je een grade set opent (bijv. Math Test 1), extra tab:
...
All edits per student

Old value → new value

Reason (optioneel comment)

Bijvoorbeeld:

John Kennedy
Feb 20 – Grade changed: 6 → 7.5
Reason: recheck test
🔁 3. Restore / Rollback

Knop:

Restore version from Feb 19

Je kan:

één student herstellen

hele grade set herstellen

Met confirm popup:

“Are you sure you want to restore this version?”

🛡️ 4. Compliance & Audit (school level)

Admins zien:

Wie publiceert grades

Wie verwijdert grades

Ongebruikelijke wijzigingen (flagged)

Alerts:

“Large grade change detected”

“Grade edited after publish”

📊 REPORTS (performance & communicatie)
...
By grade type (quiz/test/project)
...
Heatmap (weak subjects)

🧠 SMART REPORT FEATURES (advanced)

Automatic insights:

“Class average dropped compared to last test”

“5 students are at risk of failing”

Comparison mode:

This test vs previous test

This class vs other classes

Highlight outliers

🗂️ ORGANIZATION (link met jouw subject folders)

Je idee:

sorteren op subjects of tijd

Dus Reports & History werken met:

Subject folders (Math / English / Science)

Time folders (Semester 1 / Semester 2)

Combined filter:

Math → Semester 1 → Quiz reports
...
Student: only own grades history (read-only)
...
Je bouwt de volledige backend voor een educatief platform (StudyWeb / Cautie) met een hiërarchische leerstructuur vergelijkbaar met LearnBeat, maar block-based zoals Notion.

❗ De backend mag GEEN UI-aannames doen.
❗ De backend kent GEEN visuele concepten zoals “cards”, “boxes”, “balkjes”.
❗ Alles wordt aangestuurd via data, status en relaties.

De frontend rendert alles.

FUNDAMENTELE PRINCIPES (NIET AFWIJKEN)

Alles is hiërarchisch
...
Geen uitzonderingen.
Geen shortcuts.
Geen “extra” content buiten deze boom.
...
BLOCK TYPES (UITGEBREID – DIT MOET JE IMPLEMENTEREN)
1. TextBlock
{
  "content": "string",
...
➡️ Nodig voor slepen / schalen door docent
...
progress_snapshots ❗ PRECOMPUTED
student_id uuid
paragraph_id uuid
...
❌ UI-logica in backend
❌ Hardcoded nummering
❌ Hardcoded letters
❌ Live AI calls per page load
❌ Duplicate progress berekening
❌ Special cases buiten block systeem
...
🔚 EINDE PROMPT1️⃣ AI GRADING FLOW — ABSOLUUT EXPLICIET
Doel
...
Stap 1 — Student submit antwoord
...
Stap 2 — AI grading job (background task)
...
Stap 3 — Backend slaat resultaat op
...
➡️ Nooit opnieuw graden tenzij docent dit expliciet triggert

Stap 4 — Progress update

score > 0 → block telt als “completed”
...
2️⃣ VRAAG-TEMPLATES — GEDRAG + DATA + UI
...
submit → direct correct/incorrect
...
3️⃣ THEMES, KLEUREN, DARK / LIGHT / PASTEL — DIEPGAAND

Dit is waar de meeste apps falen. Jij wilt dit goed.

3.1 Kernprincipe (HEEL BELANGRIJK)

❗ Themes en talen mogen NIET elke render opnieuw berekend worden
...
➡️ Alles lage saturatie
➡️ Hoog contrast blijft behouden
...
Stap 2 — Apply via Context / CSS variables
:root {
  --bg: ...
  --text-primary: ...
}
...
Stap 4 — Re-render alleen bij wijziging
...
❌ Geen re-render per page
❌ Geen reload
❌ Geen fetch opnieuw

3.7 i18n – hoe dit correct werkt
...
➡️ Switch language = instant

4️⃣ WAT ROO CODE NU MOET BEGRIJPEN
...
Re-render alleen bij wijziging1️⃣ AI GRADING PROMPT — EXACT, STRAK, ONVERWOESTBAAR
...
🔴 AI GRADING SYSTEM PROMPT (COPY PASTE)
...
❗ Als dit schema niet klopt → backend verwerpt het resultaat.

🧠 Waarom deze prompt werkt (belangrijk voor jou, niet voor Roo)

Geen chain-of-thought

Geen discussie

Geen “het hangt ervan af”
...
2️⃣ THEME TOKEN GENERATOR — PRODUCTIE-LOGICA

Nu het tweede deel: themes correct doen, zonder rerenders, zonder spaghetti.

2.1 Kernprincipe (Roo Code MOET dit snappen)

❗ Er bestaan GEEN losse kleuren in de UI
❗ Alles werkt via tokens
❗ Een theme is data, geen styling
...
2.7 Theme toepassen — 1x render, daarna cache
App start (frontend)
...
❌ Niet bij page navigation
❌ Niet bij component mount
...
3️⃣ Wat Roo Code hier NIET fout mag doen

Zorg dat dit expliciet in je prompt staat:

❌ Geen kleuren in componenten

❌ Geen AI grading in request/response flow

❌ Geen re-render bij route change

❌ Geen vertaling via backend

❌ Geen theme logic in backend
...
🏗️ Updated Architecture Roadmap
Backend Foundation (Critical First)
Database schema - Complete hierarchical structure: classes → subjects → chapters → paragraphs → assignments → blocks
...
NEVER MENTION LEARNBEAT OR ANY OTHER EXISTING PLATFORM EVERY TO AVOID GETTING SUED.
...
how do chapoter and paragraphjs need to look:

so this is the page when you open it, there is so much useless text that shouldnt need to be there, like the path study materials to new1 the subject i am in is wrong cause what the fuck is study materals and why not sujects, also letter are incorrectly large and other even larger and sommige zijn dikgedrukt wat echt niet moet.  er staan te veel icons die niet nodig zijn, en te veel text. dan pargrafen, een paragraaf doet niks nu er staat wel paragraaf 1 maar je kan er niet op klikken om de paragrafen allemaal op een rijtje te zien je kan aleen op view assigments drukken wat ik niet eens de paragrafen laat zien alleen de assigments, de 3 paragrafen waar je het laatst aan hebt gewerkt moeten op het overzzzicht van de hoofdstukken staan met nummer als 3.7 of 3.8 en dan titel en voortgang, niet onder dus de aparte hoofdstukken maar alleen onder de vakken / subjects staan voor snelle toegang. de hoofdstuken hebben een foto toegevoegd skip ajb die kanker ai summary skip hem nu dus gewoon een foto voor nu is een placeholder net als de rest oke en dan staat er een 1 nete over die foto van 1ste hoofdstuk en een 2 voor 2de hoofdstuk . de titel staat bovenaan aan de rechterkant naast de foto boven alle paragrafeen, daaronder staan alle paragrafen genu merd als 1.1 1.2 etc met titel en een klein balkje op het eindje met ..% als het niks is 0% bv 12% of 29% of 100%. er is geen view assigments inplaats daarvan klik je gewoon op de paragraaf waar je op wil klikken , er staat nu ook bullshit met assigments en een kanker icon haal die zooi weg man. dus bijv 1.1 dan titel bv het werkwoord worden en dan whitespace tot op het einde met een ..% voortgang in de assigment zoals bijvoorbeeld vragen beantwoord en ingevuld en dan een balkje die deze procenten laat zien, het hoeft niet per 1 procent te zijn per 5 is ook ok en dat is alles. de titels van paragrafen beginnen op dezelfde plek en de nummering op dezelfd eplek en  ..% en balkjes op dezelfde plek

 so 1 teacher can edit the name of a student or teacher. by default it shows the email without the @email.com for example so johnlemon@proton.me the default would be johnlemon, but teachers are able to edit the name and it would edit in the datbase for everyone. so 1 teacher would edit the name it would show universally for all teachers and for students and in the datbase. so teachers will most liekly just name everybody once and thats it thats not a hardcoded limit that sjust my guess and intention
## Audio Integration Status (2026-05-08)

- STT is switched to **Groq Whisper primary** with OpenAI fallback in `/api/tools/transcribe`.
- TTS backend is now prepared and available at `POST /api/tools/tts`.
- TTS currently uses Groq OpenAI-compatible endpoint (`/openai/v1/audio/speech`) with env-driven model (`GROQ_TTS_MODEL`, default `playai-tts`).
- TTS is **not yet connected to any frontend tool/page**. Backend is ready; frontend wiring is pending.
- Required envs for audio:
  - `GROQ_API_KEY` (required)
  - `GROQ_TTS_MODEL` (optional, default `playai-tts`)
  - `OPENAI_STT_API_KEY` or `OPENAI_API_KEY` (optional fallback for STT)
