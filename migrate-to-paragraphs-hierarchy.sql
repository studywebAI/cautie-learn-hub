...
-- Step 1: Rename subchapters table to paragraphs (if it exists)
DO $$
BEGIN
    -- Check if subchapters table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subchapters') THEN
        -- Rename table
    ...
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'paragraphs') THEN
    ...
    ELSE
        RAISE NOTICE 'Paragraphs table already exists';
    END IF;
END $$;
